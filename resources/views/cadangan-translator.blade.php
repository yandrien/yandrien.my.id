...
<div class="pt-16 pb-16 min-h-screen flex items-start justify-center p-4">
    ...
    <div class="flex items-center flex-wrap gap-2">
        ...
        <button id="delete-word-button" class="flex items-center space-x-2 px-3 py-2 bg-red-500 text-white text-sm font-semibold rounded-lg shadow-md hover:bg-red-600 transition duration-200">
            ...
            <p class="text-green-200 text-left mt-2 text-sm">Ditenagai oleh Kamus Lokal Anda</p>
            ...
            <div class="flex flex-col md:flex-row gap-6 relative">
				...
				<div class="flex items-center justify-between mb-2">
                    <label for="input-text" id="input-label" class="block text-lg font-semibold text-gray-700 mb-2">
                        Teks Sumber (<span id="source-lang-name" class="kamuskambera">Indonesia</span>)
                    </label>
				...
				<div class="flex items-center justify-between mb-2">
                    <label for="output-text" id="output-label" class="block text-lg font-semibold text-gray-700 mb-2">
                        Hasil Terjemahan (<span id="target-lang-name">Sumba Kambera</span>)
                    </label>
                    ...
                    <textarea 
                        ...
                        class="notranslate w-full p-4 border-2 border-gray-200 bg-gray-50 rounded-lg text-gray-800 text-base cursor-default resize-none shadow-inner"
                     translate="no"></textarea>
                </div>
            </div>
        ...
        <footer class="p-4 text-center text-sm text-gray-500 border-t border-gray-100 bg-gray-50 rounded-b-xl">
            Penerjemah Sederhana V1.0. Kata yang tidak ditemukan akan dikembalikan dalam format aslinya dan dikurung siku.
        </footer>
    </div>
</div>
        ...
        <div class="p-6">
            ...
            <form id="add-word-form" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="new-id-word" required
                ...
                <div>
                    ...
                    <input type="text" id="new-sbk-word" required
                        class="mt-1 block w-full p-3 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 text-gray-800 lowercase"
                        placeholder="HARUS HURUF KECIL"
                    >
        ...
        <div class="p-6">
            ...
            <form id="edit-word-form" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="edit-id-word" required disabled
                ...
                <div>
                    ...
                    <input type="text" id="edit-sbk-word" required disabled
                        class="mt-1 block w-full p-3 border border-gray-300 bg-gray-100 rounded-lg focus:ring-yellow-500 focus:border-yellow-500 text-gray-800 lowercase"
        ...
        <div class="p-6">
            ...
            <form id="delete-word-form" class="space-y-4">
                ...
                <div class="p-4 bg-gray-50 rounded-lg border">
                    ...
                    <input type="text" id="delete-search-word" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
       ...
       // --- FUNGSI UTAMA: MENGIRIM TERJEMAHAN ---
        async function performTranslation() {
            ...
            try {
                ...
                if (data.status === 'success' && data.translation) {
					...
					// Pengecekan setting speaker dengan jeda (delay)
        ...
        // Buat versi debounced dari fungsi terjemahan, menunggu 350ms
        const debouncedTranslate = debounce(performTranslation, 350);
...
/////////////Translator bahasa asing ke indonesia sehingga kamus kambera tetap bekerja //////////////
		...
		function getActiveLanguage() {
			const labelDesk = document.getElementById('button-label-desk');
			if (!labelDesk) return 'id-ID';
			
			const currentLangText = labelDesk.innerText.trim();
			
			// 1. Cari objek bahasa di dalam array 'languages' yang namanya cocok dengan label
			const foundLang = languages.find(lang => lang.name === currentLangText);
		...
		if (deskMenuContainer || window.switchkodebahasa != 'id-ID') { 
			deskMenuContainer.addEventListener('click', function(e) {
		...
		function applyLanguageUI(kode) {
			if (kode !== 'id-ID') { 
				...
				if(sourceLangName) sourceLangName.innerText = "Bahasa Asing";
				...
				if(sourceLangName) sourceLangName.innerText = "Bahasa Indonesia";
			}
			//inputTextarea.value = "";
			//inputForeign.value = "";
			//outputTextarea.value = "";
...
// =======================================================
		...
		function setupTTS() {
			...
			/**
			* Mencari suara sistem dengan urutan prioritas: 1. id-ID/id-*, 2. en-* (Fallback Terakhir).
			* KODE INI DIPERBAIKI UNTUK MEMASTIKAN INDONESIA SELALU DIPILIH JIKA TERSEDIA.
			* @returns {SpeechSynthesisVoice | null} Suara yang ditemukan atau null.
			*
				const getIDVoice = () => {
				if (!synthesis) return null;
				const voices = synthesis.getVoices();
				
				// LANGKAH 1: PRIORITAS TINGGI - Cari Suara Indonesia
				...
				// LANGKAH 2: FALLBACK - Cari Suara Inggris (Jika Indonesia tidak ditemukan)
				selectedVoice = voices.find(voice => 
					voice.lang.startsWith('en')
				);
				
				// Jika tidak ditemukan keduanya (sangat jarang, tetapi mungkin terjadi), kembalikan null
				return selectedVoice;
			};
			...
			*/
			...
			const getDynamicVoice = (langCode) => {
				...
				// 1. Cari yang benar-benar pas (Exact Match) misal: 'ja-JP'
				let selectedVoice = voices.find(voice => voice.lang === langCode);

				// 2. Jika tidak ada, cari yang depannya sama (Partial Match) misal: 'ja'
				if (!selectedVoice) {
					const shortCode = langCode.split('-')[0];
					selectedVoice = voices.find(voice => voice.lang.startsWith(shortCode));
				}

				// 3. Jika tetap tidak ada, gunakan Indonesia sebagai fallback terakhir
				if (!selectedVoice) {
		...
		function speakText(text, buttonId) { alert(window.switchkodebahasa);
			const btn = document.getElementById(buttonId);
			const iconContainer = btn.querySelector('span') || btn;

			// JIKA SEDANG BERBICARA -> MAKA BERHENTI
			if (window.speechSynthesis.speaking) {
				window.speechSynthesis.cancel();
				// Kembali ke ikon yang sesuai status AutoRead
			...
			utterance.lang = idVoice ? idVoice.lang : window.switchkodebahasa;
		...
		speakOutputButton.addEventListener('click', () => {
			// 1. Jalankan fungsi tukar ikon & status
			toggleAutoRead();
